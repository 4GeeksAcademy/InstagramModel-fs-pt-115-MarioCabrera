...
class User(db.Model):
    ...
    def serialize(self):
        return {
            ...
            "followers": [u.username for u in self.followers],
...
class Comment(db.Model):
    id: Mapped[int] = mapped_column(primary_key=True)
    comment_text: Mapped[int] = mapped_column(String(255), nullable=False)
...
class Follower(db.Model):
    ...
    follower = relationship("User", foreign_keys=[
                            user_from_id], backref="following")
    followed = relationship("User", foreign_keys=[
                            user_to_id], backref="followers")
